import { getTeamPlayers } from './Equipes.js';


export default class Router {
    constructor(el) {
        this._el = el;
        this._elSelect = this._el.querySelector('[data-js-team]');
        this._elOptions = this._elSelect.querySelectorAll('option');
		this._elResult = this._el.querySelector('[data-js-result]');

        if (location.hostname == 'localhost') {
            let domain = `${location.pathname.substring(0, location.pathname.lastIndexOf('4-router'))}4-router/` ;
            this._domain = domain;
        } else {
    ...
    init() {
        ...
        if (team) {
            ...
            this._elOptions[team.optionIndex].selected = 'selected';
        }
        else {
        ...
        this._elSelect.addEventListener('change', function() {
            if (this._elSelect.value != 0) this.pushTeamState();
            else {
     ...
     */
    pushTeamState() {        
        let team = this.findTeam('teamId', this._elSelect.value);
